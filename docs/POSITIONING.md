...
- **Confidence.** Every estimate carries a 0–1 confidence from how tightly the distances agree
  (the residual). The twin shows a halo whose opacity tracks confidence — "roughly here", never
  fake precision. Accuracy is room-level reliably, ~1–3 m within a room with 3+ proxies; UWB/Matter
